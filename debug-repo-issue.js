  ...
  try {
    ...
    try {
      ...
      console.log("Response structure:", {
        ...
        reposLength: Array.isArray(reposResponse.data.repos)
          ? reposResponse.data.repos.length
          : "not an array",
      });
      ...
      if (reposResponse.data.repos && Array.isArray(reposResponse.data.repos)) {
        ...
        console.log(
          `Old pattern (${formattedClassName}-*): ${oldPatternRepos.length} repositories`
        );
        ...
        console.log(
          `New pattern (*-${formattedClassName}): ${newPatternRepos.length} repositories`
        );
        ...
        if (newPatternRepos.length > 0) {
          ...
          console.log(
            `Testing with repo: ${testRepo.name}, username: ${username}`
          );

          try {
            const statusResponse = await axios.post(
              `${baseURL}/api/repo/collaborationStatus`,
              {
                organizationGh,
                students: [username],
                className: className,
            ...
            console.error(
              "❌ Collaboration status check failed:",
              error.message
            );
            if (error.response) {
