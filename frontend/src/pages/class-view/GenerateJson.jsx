    ...
    const loadSavedConfig = async () => {
      ...
      try {
        ...
        if (response.data.success && response.data.data.hasConfig && response.data.data.questJsonConfig && response.data.data.questJsonConfig.questSequence && response.data.data.questJsonConfig.questSequence.length > 0) {
          ...
          console.log('📭 No existing quest JSON configuration found, keeping default quest configuration');
          ...
          console.log('💾 Auto-saving default quest configuration to backend...');
          setTimeout(() => {
            handleAutoSave();
          ...
          <Box p={3}>
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    ...
                    const { data } = await axios.post(`http://localhost:8080/api/group/${classId}/deploy-quest-to-repo`, { repo: targetRepo });
                    if (data && data.success) {
                      alert(`Deployed ${data.data.questId} to ${targetRepo}. Issue: ${data.data.issueUrl}`);
                    ...
                    alert('Failed to deploy quest to repo. Check backend logs.');
                  }
                }}
                sx={{ borderRadius: 3, color: '#ff5722', borderColor: '#ff5722', '&:hover': { borderColor: '#e64a19', backgroundColor: '#fff3e0' } }}
              >
                Deploy MCQ Quest to Repo
              </Button>
              <Typography variant="caption" color="text.secondary">
                Adds a bonus MCQ quest (prereq Q0) to OSS-Doorway-Dev/MisanEtchie-financing
              </Typography>
            </Box>
            ...
            <Box sx={{ mt: 3 }}>
              {storedKeys && storedKeys.length > 0 ? (
                ...
                <Typography variant="body2" color="text.secondary">Values are recorded by the bot at runtime per student and will appear here when available.</Typography>
              )}
            </Box>
          </Box>
        </Card>
