...
const ManageQuests = () => {
  ...
  const removeDefaultQuests = () => {
    setUnifiedQuestOrder(prevOrder => {
      const newOrder = prevOrder.filter(quest => !['Q1', 'Q2', 'Q3'].includes(quest.id));
      debouncedSaveQuestOrder(newOrder);
      return newOrder;
    });
    setQuestOrderSaveStatus('Q1, Q2, Q3 removed from quest order. They are now available in the quest library.');
    setTimeout(() => setQuestOrderSaveStatus(''), 3000);
  };
  ...
  const addDefaultQuestToClass = (questId) => {
    const defaultQuest = {
      ...
      title: questId,
      questTitle: questId,
      content: questId === 'Q1' ? 'Understanding OSS Projects and GitHub Basics' :
               questId === 'Q2' ? 'Forking and Contributing to Repositories' :
    ...
    const questData = {
      ...
      tasks: quest.tasks ? quest.tasks.map(task => {
        ...
        const taskData = {
          ...
          descriptionImage: null,
          // JSON format fields
          desc: task.desc || task.description || task.taskTitle || task.title || '',
  ...
  const handleDeleteQuest = async (quest) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert(`Quest "${quest.questTitle}" has been deleted permanently from the database!`);
      }
    } catch (error) {
  ...
  const handleRemoveQuestFromClass = (quest) => {
    if (!window.confirm(`Are you sure you want to remove "${quest.questTitle}" from this class? The quest will remain in your quest library.`)) {
      return;
    }
  ...
  const handleAddQuestToClass = async (quest) => {
    try {
      ...
      const isAlreadyInClass = unifiedQuestOrder.some(q => q._id === quest._id);
      if (isAlreadyInClass) {
        alert(`Quest "${quest.questTitle}" is already in this class's quest order.`);
        return;
      }
      ...
      alert(`Quest "${quest.questTitle}" has been added to this class's quest order!`);
    } catch (error) {
  ...
  const handleUploadMCQQuest = async () => {
    ...
    const validTasks = questFormData.tasks.filter(task => 
      task.title.trim() && 
      task.desc.trim() && 
    ...
    if (validTasks.length === 0) {
      alert('Please add at least one task with a title, description, and task description.');
      return;
    }
    
    if (validTasks.length !== questFormData.tasks.length) {
      alert('Some tasks are missing required fields. Please fill in all task information.');
      return;
    }
    ...
    try {
      ...
      setUploadQuestStatus(`❌ Error uploading quest: ${error.response?.data?.message || error.message}`);
    } finally {
  ...
  const handleDownloadJSON = () => {
    try {
      ...
      const questConfig = {
        ...
        questSequence: unifiedQuestOrder.map((quest, index) => {
          ...
          if (quest.type === 'custom' && quest._id) {
            ...
            if (customQuest) {
              questData.badgeDescription = customQuest.badgeDescription || "Custom Quest 🎯";
              questData.metadata.title = quest.title || quest.questTitle;
              questData.metadata.description = quest.content || quest.description || customQuest.description || '';
      ...
      link.href = url;
      link.download = `quest-config-${classInfo.groupName || 'class'}-${new Date().toISOString().split('T')[0]}.json`;
  ...
  return (
    <Container maxWidth="md" sx={{ py: 4 }}>
      ...
      <Stack direction="row" spacing={2} mb={4}>
        <Button variant="contained" sx={{ bgcolor: '#fb5233', '&:hover': { bgcolor: '#e04a2e' } }} onClick={() => setShowQuestModal(true)}>
          Create Quest
        </Button>
        <Button variant="contained" color="success" onClick={handleViewMyQuests} disabled={isLoadingQuests}>
          {isLoadingQuests ? 'Loading...' : 'View Quest Library'}
        </Button>
        <Button variant="contained" color="secondary" onClick={resetQuestOrderToDefault} disabled={isSavingQuestOrder}>
      ...
      <Box sx={{ minHeight: 60, mb: 3, display: 'flex', alignItems: 'center' }}>
        {questOrderSaveStatus && (
          <Alert 
            severity={questOrderSaveStatus.includes('Error') ? 'error' : questOrderSaveStatus.includes('successfully') ? 'success' : 'info'} 
            sx={{ width: '100%' }}
          >
            {questOrderSaveStatus}
          </Alert>
      ...
      <Box>
        {unifiedQuestOrder.map((quest, index) => (
          <Card key={quest._id || quest.id} sx={{ mb: 2, p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: quest.isQ0 ? '#f5f5f5' : 'white' }}>
            <Box>
                ...
                <Box sx={{ ml: 4, mt: 2 }}>
                  {Array.isArray(quest.tasks)
                    ? quest.tasks.map((task, tIdx) => (
                        <Stack key={tIdx} direction="row" alignItems="center" spacing={2} sx={{ mb: 1 }}>
                          <Chip
                            label={
                          ...
                          <Chip label={`XP: ${task.xp ?? 0}`} color="success" size="small" sx={{ mr: 1 }} />
                          <Chip label={`Points: ${task.points ?? 0}`} color="primary" size="small" />
                        </Stack>
                      ))
                    : Object.entries(quest.tasks).map(([taskKey, task]) => (
                        <Stack key={taskKey} direction="row" alignItems="center" spacing={2} sx={{ mb: 1 }}>
                          <Chip
                            label={
                              task.type === 'multiple-choice' ? 'Multiple Choice'
                              : task.type === 'get-issue-count' && task.ossRepository ? 'Get Issue Count'
            ...
            <Stack direction="row" spacing={1} alignItems="center">
              {!quest.isQ0 && (
                <>
                  ...
                  <button className="btn btn-outline-secondary btn-sm" title="Move Up" onClick={() => quest.type === 'fixed' ? moveFixedQuestUp(quest.id) : moveQuestUp(quest._id)}>
                    ↑
                  </button>
                  <button className="btn btn-outline-secondary btn-sm" title="Move Down" onClick={() => quest.type === 'fixed' ? moveFixedQuestDown(quest.id) : moveQuestDown(quest._id)}>
                    ↓
                  </button>
                  {quest.type === 'custom' && (
                    <Button variant="outlined" color="primary" size="small" onClick={() => handleEditQuest(quest)}>
                      ✏️ Edit
                    </Button>
                  )}
                  {quest.type === 'custom' && (
                    <Button variant="outlined" color="error" size="small" onClick={() => handleRemoveQuestFromClass(quest)}>
                      🗑️ Remove from Class
                    </Button>
                  )}
                  {quest.type === 'fixed' && (
                    <Button variant="outlined" color="error" size="small" onClick={() => {
                      setUnifiedQuestOrder(prevOrder => {
                        const newOrder = prevOrder.filter(q => q.id !== quest.id);
                        debouncedSaveQuestOrder(newOrder);
                        return newOrder;
                      });
                      setQuestOrderSaveStatus(`${quest.id} removed from quest order. It's available in the quest library.`);
                      setTimeout(() => setQuestOrderSaveStatus(''), 3000);
                    }}>
                      🗑️ Remove from Class
                    </Button>
              ...
              {quest.isQ0 && (
                <Typography variant="caption" color="secondary" sx={{ ml: 1 }}>Fixed</Typography>
              )}
            </Stack>
          </Card>
        ))}
      ...
      <Box sx={{ mt: 4, p: 3, border: '1px solid #e0e0e0', borderRadius: 2, bgcolor: '#fafafa' }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          Download the current quest order and configuration as a JSON file. This includes all quests in their current order, metadata, and task information.
        </Typography>
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Typography variant="caption" color="text.secondary">
            {unifiedQuestOrder.length} quests • {unifiedQuestOrder.filter(q => q.type === 'custom').length} custom • {unifiedQuestOrder.filter(q => q.type === 'fixed').length} default
          </Typography>
        </Stack>
      </Box>
      ...
      <Dialog open={showQuestModal} onClose={isEditMode ? handleCancelEdit : () => setShowQuestModal(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          <Box component="form" sx={{ mt: 1 }}>
            ...
            <Typography variant="h6" sx={{ mb: 2 }}>Basic Quest Information</Typography>
            <TextField
              fullWidth
              label="Quest ID (e.g., Module1, Q1, etc.)"
              value={questFormData.questId}
              onChange={e => setQuestFormData({ ...questFormData, questId: e.target.value })}
              margin="normal"
            ...
            <TextField
              ...
              onChange={e => setQuestFormData({ ...questFormData, badgeDescription: e.target.value })}
              margin="normal"
              multiline
              rows={2}
            ...
            {/* Quest Metadata */}
            <Typography variant="h6" sx={{ mt: 3, mb: 2 }}>Quest Metadata</Typography>
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Prerequisites will be automatically generated based on quest order when you download the configuration.
            </Typography>
            ...
            {questFormData.tasks.map((task, taskIndex) => (
              <Card key={taskIndex} sx={{ mb: 3, p: 2 }}>
                <Stack direction="row" spacing={2} alignItems="center" mb={2}>
                  ...
                  {questFormData.tasks.length > 1 && (
                    <Button color="error" onClick={() => {
                      const tasks = questFormData.tasks.filter((_, i) => i !== taskIndex);
                      setQuestFormData({ ...questFormData, tasks });
                    }}>Remove</Button>
                  )}
                </Stack>
                    ...
                    <Stack direction="row" spacing={2}>
                      <TextField
                        ...
                        onChange={e => {
                          const tasks = [...questFormData.tasks];
                          if (!tasks[taskIndex].config) tasks[taskIndex].config = {};
                          tasks[taskIndex].config.questionCount = parseInt(e.target.value) || 5;
                          setQuestFormData({ ...questFormData, tasks });
                        }}
                        sx={{ width: 200 }}
                      ...
                      <TextField
                        ...
                        onChange={e => {
                          const tasks = [...questFormData.tasks];
                          if (!tasks[taskIndex].config) tasks[taskIndex].config = {};
                          tasks[taskIndex].config.correctAnswers = e.target.value;
                          setQuestFormData({ ...questFormData, tasks });
                        }}
                        sx={{ flex: 1 }}
                ...
                {(task.type === 'issue-selection' || task.type === 'pr-creation') && (
                  <TextField
                    fullWidth
                ...
                <Typography variant="subtitle1" sx={{ mt: 3, mb: 1 }}>Response Messages</Typography>
                <TextField
                  fullWidth
                ...
                {/* Task Hints Section */}
                <Typography variant="subtitle1" sx={{ mt: 3, mb: 1 }}>Hints</Typography>
                <Stack direction="row" alignItems="center" spacing={2} mb={2}>
                  <Button
                    variant={(task.hints && task.hints.length > 0) ? 'contained' : 'outlined'}
                    color="primary"
                    onClick={() => {
                      const tasks = [...questFormData.tasks];
                  ...
                  >
                    {(task.hints && task.hints.length > 0) ? 'Hints Enabled' : 'Enable Hints'}
                  </Button>
                </Stack>
                {task.hints && task.hints.length > 0 && (
                  <>
                    {task.hints.map((hint, hintIndex) => (
                      <Card key={hintIndex} sx={{ mb: 2, p: 2, bgcolor: 'grey.50' }}>
                        <Stack direction="row" spacing={2} alignItems="center" mb={2}>
                          <TextField
                            label="Sequence"
                            type="number"
                            ...
                            onChange={e => {
                              const tasks = [...questFormData.tasks];
                              tasks[taskIndex].hints[hintIndex].sequence = parseInt(e.target.value) || 1;
                              setQuestFormData({ ...questFormData, tasks });
                            }}
                            sx={{ width: 120 }}
                            inputProps={{ min: 1 }}
                          ...
                          <TextField
                            ...
                            onChange={e => {
                              const tasks = [...questFormData.tasks];
                              tasks[taskIndex].hints[hintIndex].penalty = parseInt(e.target.value) || 0;
                              setQuestFormData({ ...questFormData, tasks });
                            }}
                            sx={{ width: 150 }}
                            inputProps={{ min: 0 }}
                          ...
                          <Button color="error" onClick={() => {
                            const tasks = [...questFormData.tasks];
                            tasks[taskIndex].hints = tasks[taskIndex].hints.filter((_, i) => i !== hintIndex);
                            setQuestFormData({ ...questFormData, tasks });
                          }}>Remove</Button>
                        </Stack>
                        <TextField
                          ...
                          onChange={e => {
                            const tasks = [...questFormData.tasks];
                            tasks[taskIndex].hints[hintIndex].content = e.target.value;
                            setQuestFormData({ ...questFormData, tasks });
                          }}
                          multiline
                          rows={2}
            ...
            {/* Hints Section */}
            <Box mt={4}>
              <Typography variant="h6" sx={{ mb: 1 }}>Hints</Typography>
              <Stack direction="row" alignItems="center" spacing={2} mb={2}>
                <Button
                  variant={questFormData.hints.enabled ? 'contained' : 'outlined'}
                  color="primary"
                  onClick={() => setQuestFormData({ ...questFormData, hints: { ...questFormData.hints, enabled: !questFormData.hints.enabled } })}
                >
                  {questFormData.hints.enabled ? 'Hints Enabled' : 'Enable Hints'}
                </Button>
                {questFormData.hints.enabled && (
                  <TextField
                    ...
                    onChange={e => setQuestFormData({ ...questFormData, hints: { ...questFormData.hints, penalty: parseInt(e.target.value) || 0 } })}
                    sx={{ width: 160 }}
                  />
                )}
              </Stack>
              {questFormData.hints.enabled && (
                <>
                  {questFormData.hints.hints.map((hint, hintIndex) => (
                    <Stack direction="row" spacing={2} alignItems="center" key={hintIndex} mb={1}>
                      <TextField
                        ...
                        onChange={e => {
                          ...
                          setQuestFormData({ ...questFormData, hints: { ...questFormData.hints, hints } });
                        }}
                        sx={{ flex: 1 }}
                      />
                      <Button color="error" onClick={() => {
                        const hints = questFormData.hints.hints.filter((_, i) => i !== hintIndex);
                        setQuestFormData({ ...questFormData, hints: { ...questFormData.hints, hints } });
                      }}>Remove</Button>
                    </Stack>
                  ))}
        ...
        <DialogActions>
          <Button onClick={isEditMode ? handleCancelEdit : () => setShowQuestModal(false)} color="secondary">
        ...
        {uploadQuestStatus && (
          <Box px={3} pb={2} width="100%">
            <Alert severity={uploadQuestStatus.startsWith('✅') ? 'success' : 'error'}>{uploadQuestStatus}</Alert>
          </Box>
        )}
      </Dialog>
      ...
      <Dialog open={showQuestsModal} onClose={() => setShowQuestsModal(false)} maxWidth="lg" fullWidth>
        ...
        <DialogContent>
                    ...
                    return (
                      <Alert severity="info" sx={{ mb: 2 }}>
                        ...
                        <Typography variant="body2">
                          {questsInClass.length} of {myQuests.length} custom quests are already in this class's quest order.
                        </Typography>
                      </Alert>
                    );
                  }
                              ...
                              <Box textAlign="right">
                                ...
                                <Stack direction="row" spacing={1} mt={1}>
                                  ...
                                  <Button 
                                    ...
                                    title={isAlreadyInClass ? "Quest is already in this class" : "Add quest to this class"}
                                  >
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              These are the standard quests that come with the system. They can be added to your class but cannot be edited.
            </Typography>
            
            <Grid container spacing={2}>
              {[
                { id: 'Q1', title: 'Q1: Understanding OSS Projects and GitHub Basics', content: 'Learn the fundamentals of open source software and GitHub workflow' },
                { id: 'Q2', title: 'Q2: Forking and Contributing to Repositories', content: 'Master the process of forking repositories and making contributions' },
                { id: 'Q3', title: 'Q3: Creating Pull Requests and Code Reviews', content: 'Learn how to create pull requests and participate in code reviews' }
              ].map((defaultQuest) => {
                const isAlreadyInClass = unifiedQuestOrder.some(q => q.id === defaultQuest.id);
                return (
                        ...
                        <Box display="flex" justifyContent="space-between" alignItems="flex-start">
                          ...
                          <Box textAlign="right">
                            ...
                            <Button 
                              ...
                              color="secondary"
                              size="small"
                              onClick={() => addDefaultQuestToClass(defaultQuest.id)}
                              disabled={isAlreadyInClass}
                              title={isAlreadyInClass ? "Quest is already in this class" : "Add default quest to this class"}
                              sx={{ mt: 1 }}
                            >
                              {isAlreadyInClass ? "✓ Added" : "Add to Class"}
                            </Button>
