...
const ClassView = () => {
  ...
  console.log(
    "🔍 [ClassView] classId length:",
    classId ? classId.length : "null"
  );
  ...
  const [fixedQ0Quest] = useState({
    ...
    description:
      "Welcome to the world of Open Source Software! This foundational quest will introduce you to the basic concepts and tools you'll need throughout this course.",
    content:
      "This is the fixed introductory quest that cannot be modified or deleted. It serves as the foundation for all other quests in this course.",
    type: "fixed",
  });
  ...
  const fetchClassInfo = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/group/class/${classId}`
      );
  ...
  const fetchOrganizationRepos = async () => {
    try {
      ...
      console.log("📥 [ClassView] Response structure:", {
        ...
        reposLength: Array.isArray(response.data.repos)
      ...
      if (response.data && Array.isArray(response.data.repos)) {
        ...
        console.log(
          `📊 [ClassView] Total repositories fetched: ${response.data.repos.length}`
        );
        ...
        console.log(
          "🔍 [ClassView] Looking for repositories ending with:",
          `-${formattedClassName}`
        );
        ...
        console.log(
          `✅ [ClassView] Found ${classRepos.length} repositories for this class`
        );
        console.log(
          "👥 [ClassView] Class repositories:",
          classRepos.map((r) => r.githubUsername)
        );
        ...
        if (classRepos.length > 0) {
          try {
            const statusResponse = await axios.post(
              `${API_BASE_URL}/api/repo/collaborationStatus`,
              {
                organizationGh,
            ...
            if (statusResponse.data.results) {
              console.log(
                "✅ [ClassView] Collaboration status updated:",
            ...
            console.error(
              "❌ [ClassView] Error checking collaboration status:",
              error
            );
          }
        }
        ...
        console.error(
          "❌ [ClassView] Invalid response structure:",
          response.data
        );
      }
    } catch (error) {
      console.error(
        "❌ [ClassView] Error fetching organization repositories:",
        error
      );
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      if (
        ...
        setCreateReposStatus(
          `Error: Invalid class ID format. Expected 24-character hex string, got: ${classId}`
        );
        return;
      }
      ...
      reader.onload = async (e) => {
        ...
        console.log(
          "🔍 [CSV-UPLOAD] Using createCustomRepos for each user individually"
        );
        ...
        for (let i = 0; i < usernames.length; i++) {
          ...
          setCreateReposStatus(
            `Processing ${i + 1}/${
              usernames.length
            }: Creating repository for ${username}...`
          );
          ...
          try {
            ...
            if (myQuests.length === 0) {
              console.log(
                "🔄 [QUEST-CONFIG] Loading myQuests for task data..."
              );
              try {
                const response = await axios.get(
                  `${API_BASE_URL}/api/quest/professor/${authUser._id}`
                );
                if (response.data.success) {
                  setMyQuests(response.data.data);
                  console.log(
                    "✅ [QUEST-CONFIG] Loaded myQuests:",
                ...
                console.error(
                  "❌ [QUEST-CONFIG] Error loading myQuests:",
                  error
                );
              }
            }
            ...
            if (!generateJsonConfig && !questConfig) {
              setCreateReposStatus(
                "❌ No quest configuration found. Please set up quests in GenerateJson or ManageQuests."
              );
              return;
            }
            ...
            console.log(
              `🎮 [CUSTOM-QUESTS] Quest configuration being sent to bot for ${username}:`
            );
            console.log(
              `🎮 [CUSTOM-QUESTS] Full questConfig:`,
              JSON.stringify(requestBody.customSequence, null, 2)
            );

            if (requestBody.customSequence?.questSequence) {
              console.log(`🎮 [CUSTOM-QUESTS] Quest sequence breakdown:`);
              requestBody.customSequence.questSequence.forEach(
                (quest, index) => {
                  ...
                  if (quest.questType === "custom") {
                    console.log(
                      `🔥 [CUSTOM-QUEST-DETAIL] Custom Quest "${quest.title}":`,
                      {
                        questId: quest.questId,
                    ...
                    if (quest.tasks) {
                      console.log(
                        `📋 [CUSTOM-QUEST-TASKS] Tasks for "${quest.title}":`
                      );
              ...
              const customQuests =
                requestBody.customSequence.questSequence.filter(
              ...
              console.log(
                `🎯 [BOT-SUMMARY] Bot will receive for ${username}:`,
                {
  ...
  const fetchStudentScores = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/repo/studentScores`,
        {
          className: classInfo.groupName,
          students: studentData.map((repo) => repo.githubUsername),
        }
      ...
      reader.onload = (e) => {
        ...
        if (type === "description") {
          ...
          setQuestFormData((prev) => ({
            ...
            tasks: prev.tasks.map((task, index) =>
              index === taskIndex
                ? { ...task, descriptionImage: imageData }
                : task
            ),
          }));
        }
  ...
  const handleReuploadReadme = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setReuploadStatus(
          "README re-uploaded and pushed to all student repositories successfully!"
        );
    ...
    try {
      ...
      const questData = {
        ...
        tasks: questFormData.tasks.map((task, index) => ({
          ...
          hints: questFormData.hints.enabled
      ...
      if (isEditMode && editingQuest) {
        ...
        response = await axios.put(
          `${API_BASE_URL}/api/quest/${editingQuest._id}`,
          questData
        );
      } else {
        // Create new quest
        response = await axios.post(
          `${API_BASE_URL}/api/quest/upload-mcq`,
          questData
        );
      }
    ...
    const questData = {
      ...
      tasks: quest.tasks
        ? quest.tasks.map((task, taskIndex) => {
            console.log(
              `📋 [EDIT-TASK-${taskIndex + 1}] Loading task data:`,
              task
            );
            ...
            console.log(
              `✅ [EDIT-TASK-${taskIndex + 1}] Mapped form data:`,
              formTask
            );
            return formTask;
          })
  ...
  const handleDeleteQuest = async (quest) => {
    ...
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/api/quest/${quest._id}`
      );
      if (response.data.success) {
  ...
  const moveQuestUp = (questId) => {
    ...
    setUnifiedQuestOrder((prevOrder) => {
      console.log(
        "Previous unified order:",
        prevOrder.map((q) => q.title || q.questTitle)
      );
      ...
      if (currentIndex > 1) {
        // Can't move above Q0 (index 0) or Q1 (index 1)
        console.log(
          `Moving "${
            newOrder[currentIndex].title || newOrder[currentIndex].questTitle
          }" from position ${currentIndex} to ${currentIndex - 1}`
        );
        ...
        console.log(
          "New unified order:",
          newOrder.map((q) => q.title || q.questTitle)
        );
  ...
  const loadQuestOrderFromDatabase = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/api/group/${classId}/quest-order`
      );
      ...
      if (response.data.questOrder) {
        ...
        const fullQuestDataResponse = await axios.get(
          `${API_BASE_URL}/api/quest/professor/${authUser._id}`
        );
      ...
      console.error(
        "❌ [DEBUG] Error loading quest order from database:",
        error
      );
      console.error(
        "❌ [DEBUG] Full error:",
  ...
  const saveQuestOrderToDatabase = async (questOrder, retryCount = 0) => {
    ...
    try {
      ...
      console.log(
        "Quest order and prerequisites saved to database:",
        response.data
      );
      ...
      if (error.response?.status === 409) {
        ...
        if (retryCount < 3) {
          console.log(
            `Version conflict detected, retrying... (attempt ${retryCount + 1})`
          );
          setQuestOrderSaveStatus(
            `Retrying due to concurrent changes... (attempt ${retryCount + 1})`
          );
          ...
          setQuestOrderSaveStatus(
            "Quest order was modified by another operation. Please refresh and try again."
          );
        ...
        if (
          errorMessage.includes("No matching document found") &&
          retryCount < 3
        ) {
          console.log(
            `Retrying quest order save (attempt ${retryCount + 1})...`
          );
  ...
  const resetQuestOrderToDefault = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/group/${classId}/quest-order/reset`
      );
      if (response.data.success) {
        loadQuestOrderFromDatabase();
  ...
  const questConfig = {
    ...
    questSequence: unifiedQuestOrder.map((quest, index) => {
      ...
      if (quest.type === "fixed") {
        ...
        console.log(
          `🔍 [QUEST-CONFIG] Processing custom quest ${quest._id}:`,
          quest
        );
        ...
        if (!realTasks && quest.type === "custom" && quest._id) {
          ...
          if (customQuest && customQuest.tasks) {
            console.log(
              `✅ [QUEST-CONFIG] Found task data in myQuests for ${quest._id}:`,
              customQuest.tasks
            );
        ...
        if (realTasks && Array.isArray(realTasks) && realTasks.length > 0) {
          console.log(
            `🎯 [QUEST-CONFIG] Using real task data for ${quest._id}:`,
            realTasks
          );
          ...
          realTasks.forEach((task, taskIndex) => {
            ...
            else if (task.description && task.description !== "N/A")
              taskDesc = task.description;
            else if (task.taskTitle && task.taskTitle !== "N/A")
              taskDesc = task.taskTitle;
            ...
            const acceptResponse =
              task.responses?.accept || task.accept || "Complete this task";
            const successResponse =
              task.responses?.success ||
              task.success ||
              "Task completed successfully!";
            const errorResponse =
              task.responses?.error ||
              task.error ||
          ...
          console.log(
            `✅ [QUEST-CONFIG] Transformed tasks for ${quest._id}:`,
            transformedTasks
          );
        } else if (
          realTasks &&
          typeof realTasks === "object" &&
          Object.keys(realTasks).length > 0
        ) {
          console.log(
            `🎯 [QUEST-CONFIG] Using object task data for ${quest._id}:`,
            realTasks
          );
          questData.tasks = realTasks;
        } else {
          console.warn(
            `⚠️ [QUEST-CONFIG] No task data found for custom quest ${quest._id}, using fallback`
          );
  ...
  const questBreakdownQuests =
    generateJsonConfig && generateJsonConfig.questSequence
      ? generateJsonConfig.questSequence
      : unifiedQuestOrder;

  // Calculate total possible points for the class
  const totalPossiblePoints = (questBreakdownQuests || []).reduce(
    (sum, quest) => {
      if (quest.tasks) {
        if (Array.isArray(quest.tasks)) {
          return (
            sum + quest.tasks.reduce((tSum, t) => tSum + (t.points || 0), 0)
          );
        } else {
          return (
            sum +
            Object.values(quest.tasks).reduce(
              (tSum, t) => tSum + (t.points || 0),
              0
            )
          );
        }
    ...
    <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <Box sx={{ p: 1 }}>
          <Button
            ...
            sx={{
              ...
              "&:hover": {
                bgcolor:
                  currentView === "dashboard" ? "#e64a19" : "rgba(0,0,0,0.04)",
              },
            }}
          >
          ...
          <Button
            ...
            sx={{
              ...
              px: 2,
              bgcolor:
                currentView === "manage-quests" ? "#fb5233" : "transparent",
          ...
          <Button
            ...
            sx={{
              ...
              px: 2,
              bgcolor:
                currentView === "manage-students" ? "#fb5233" : "transparent",
              color:
                currentView === "manage-students" ? "white" : "text.primary",
              borderRadius: 3,
              "&:hover": {
                bgcolor:
      ...
      >
        ...
        {currentView === "dashboard" ? (
          <Container
            maxWidth="lg"
            sx={{ py: 4, width: "100%", textAlign: "left" }}
          >
            {!classId ? (
              <Box
                display="flex"
                        ...
                        {(() => {
                          ...
                          Object.values(studentScores).forEach((scores) => {
                            ...
                            if (scores.questProgress) {
                              Object.values(scores.questProgress).forEach(
                                (questProgress) => {
                                  if (questProgress.tasks) {
                                    Object.entries(questProgress.tasks).forEach(
                                      ([taskId, task]) => {
                                        if (
                                          task.completed &&
                                          !studentCompletedTaskIds.has(taskId)
                                        ) {
                                          totalCompletedTasks++;
                          ...
                          const totalPossibleTasks =
                            totalTasks * studentData.length;
                          const overallCompletionRate =
                            totalPossibleTasks > 0
                              ? Math.round(
                                  (totalCompletedTasks / totalPossibleTasks) *
                                    100
                                )
                              : 0;
                    ...
                    {isLoading ? (
                      ...
                      <Box textAlign="center" py={4}>
                        <Typography color="text.secondary">
                          No student repositories found. Click "Add Students" to
                          create repositories!!
                        </Typography>
                      </Box>
                    ) : (
                      ...
                      <Box>
                        ...
                        <Box>
                          {studentData
                            .sort((a, b) => {
                              const scoreA =
                                studentScores[a.githubUsername]?.points || 0;
                              const scoreB =
                                studentScores[b.githubUsername]?.points || 0;
                              return scoreB - scoreA; // Sort by points descending
                            })
                            .map((student, index) => {
                              const scores =
                                studentScores[student.githubUsername] || {};
                              ...
                              const totalTasks = (
                                ...
                                if (quest.tasks) {
                                  if (Array.isArray(quest.tasks)) {
                                    ...
                                    return (
                                      sum + Object.keys(quest.tasks).length
                                    );
                                  }
                                }
                                return sum;
                    ...
                    >
                      ...
                      {(() => {
                        ...
                        <Box>
                          ...
                          <Box>
                            {questBreakdownQuests.map((quest, questIndex) => {
                              const questId =
                                quest.id || quest._id || `Q${questIndex}`;
                              const questTitle =
                                quest.title || quest.questTitle || questId;
                              ...
                              studentData.forEach((student) => {
                                ...
                                if (scores) {
                                  ...
                                  const classQuestId = `${(
                                    classInfo.groupName || ""
                                  ).replace(/[^a-zA-Z0-9]+/g, "")}-${questId}`;
                                  const possibleQuestKeys = [
                                    questId,
                                    `${(classInfo.groupName || "").replace(
                                      /[^a-zA-Z0-9]+/g,
                                      ""
                                    )}-${questId}`,
                                    questTitle,
                              ...
                              const completionRate =
                                studentData.length > 0
                                  ? Math.round(
                                      (completedCount / studentData.length) *
                                        100
                                    )
                                  : 0;
                              const avgScore =
                                studentsWithData > 0
                                  ? Math.round(totalScore / studentsWithData)
                                  : 0;
                              const avgXP =
                                studentsWithData > 0
                                  ? Math.round(totalXP / studentsWithData)
                                  : 0;
                        ...
                        <Typography
                          ...
                          sx={{ mt: 2 }}
                        >
                          No quests configured yet. Use "Manage Quests" to
                          create quests for this class.
                        </Typography>
                      )}
                    </Box>
                  </Box>
                ...
                {currentProcessingUser && (
                  <Alert severity="info" sx={{ mb: 2 }}>
                    <Typography variant="body2">
                      ...
                      <strong>{currentProcessingUser}</strong>
                    </Typography>
                  </Alert>
                )}
                ...
                {processedCount === totalUsers && (
                  <Alert severity="success" sx={{ mt: 2 }}>
                    <Typography variant="body2">
                      ...
                      {processingResults.successful.length} successful,{" "}
                      {processingResults.unsuccessful.length} failed.
                    </Typography>
                    <Typography variant="body2" sx={{ mt: 1 }}>
                      ⏳ Refreshing collaboration status... This dialog will
                      close automatically.
                    </Typography>
                  </Alert>
                )}
        ...
        >
          ...
          <DialogContent>
            <Alert severity="info" sx={{ mb: 3 }}>
              ...
              <Typography variant="body2">
                Upload a README file that will be automatically included in the
                initial commit when creating student repositories. This README
                will contain course information, objectives, and instructions
                for students.
              </Typography>
            </Alert>
            ...
            {existingReadme && (
              <Alert severity="warning" sx={{ mb: 3 }}>
                ...
                <Typography variant="body2">
                  This class already has a README file:{" "}
                  <strong>{existingReadme.fileName}</strong>
                </Typography>
                <Typography variant="body2">
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Typography variant="body2" paragraph>
                Upload a markdown (.md) file that will serve as the README for
            ...
            <Box sx={{ mb: 3 }}>
              ...
              {readmeContent ? (
                ...
                <Paper
                  sx={{ p: 2, bgcolor: "grey.50", color: "text.secondary" }}
                >
                  <Typography variant="body2">
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Typography variant="body2" paragraph>
                Don't have a README file? Download this template as a starting
                point:
              </Typography>
...
*This README is automatically updated as you progress through the course.*`;
            ...
            {existingReadme && (
              ...
              >
                {isReuploading
                  ? "Re-uploading..."
                  : "Re-upload README & Push to Student Repos"}
              </Button>
            )}
            {!existingReadme && (
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    console.log(
                      "📝 [README-UPLOAD] Starting README upload process..."
                    );
                    ...
                    console.log(
                      "✅ [README-UPLOAD] README upload successful:",
                      {
                        status: response.status,
                        data: response.data,
                      }
                    ...
                    if (response.status === 200) {
                      console.log(
                        "🎉 [README-UPLOAD] README saved successfully to database"
                      );
                      alert(
                        "README file saved successfully! It will be added to all new student repositories."
                      );
                      setShowReadmeModal(false);
                      setReadmeFile(null);
                      setReadmeContent("");
                      ...
                      console.error(
                        "⚠️ [README-UPLOAD] Unexpected response status:",
                        response.status
                      );
                    ...
                    console.error(
                      "❌ [README-UPLOAD] README upload failed:",
                      error
                    );
        ...
        >
          ...
          <DialogContent>
            <Typography variant="body2" color="text.secondary" paragraph>
              Quest creation form would go here. This is a placeholder for the
              enhanced quest form.
            </Typography>
          </DialogContent>
          <DialogActions>
        ...
        {/* Batch Repo Dialog */}
        <Dialog
          open={showBatchRepoDialog}
          onClose={() => setShowBatchRepoDialog(false)}
          maxWidth="sm"
          fullWidth
        >
          ...
          <DialogContent>
            <Typography gutterBottom>
              Upload a CSV file with a list of GitHub usernames (one per line,
              or with a header 'github_username').
            </Typography>
            <input
              accept=".csv"
              type="file"
          ...
          <DialogActions>
            <Button onClick={() => setShowBatchRepoDialog(false)}>
              Cancel
            </Button>
            <Button
        ...
        >
          ...
          <DialogContent>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
              Students can use this link to access the invite page where they
              can enter their GitHub username to create a repository.
            </Typography>
          </DialogContent>
          <DialogActions>
