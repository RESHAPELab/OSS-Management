...
export default function CreateCustomRepoCSVBatch({
  customQuestConfig,
  classInfo,
}) {
  ...
  const checkRepositoryStatus = async () => {
    ...
    try {
      const usernames = batchResults.successful.map(
        (result) => result.username
      );
      console.log(
        "👥 [STATUS-CHECK] Checking status for usernames:",
        usernames
      );
      ...
      console.error(
        "💥 [STATUS-CHECK] Error checking repository status:",
        error
      );
  ...
  const processBatchCreateRepos = async () => {
    ...
    if (!customQuestConfig) {
      alert(
        "No custom quest arrangement provided. Please configure quests first."
      );
      return;
    }
    ...
    if (!classInfo?.groupName) {
      alert(
        "Class information not available. Please refresh the page and try again."
      );
      return;
    }
    ...
    console.log(
      "🏷️ [CSV-BATCH] Repository naming pattern:",
      classInfo.groupName
    ...
    try {
      ...
      console.log(
        `📝 [CSV-BATCH] Parsed ${usernames.length} usernames from CSV:`,
        usernames
      );
      ...
      console.log(
        `🚀 [CSV-BATCH] Beginning individual repository creation for ${usernames.length} users`
      );
      ...
      for (let i = 0; i < usernames.length; i++) {
        ...
        try {
          ...
          console.log(
            `🎮 [BATCH-CUSTOM-QUESTS] Full customQuestConfig:`,
            JSON.stringify(questConfigWithRealTasks, null, 2)
          );

          if (questConfigWithRealTasks?.questSequence) {
            console.log(
              `🎮 [BATCH-CUSTOM-QUESTS] Quest sequence breakdown for ${username}:`
            );
            questConfigWithRealTasks.questSequence.forEach((quest, index) => {
              ...
              if (quest.questType === "custom") {
                console.log(
                  `🔥 [BATCH-CUSTOM-QUEST-DETAIL] Custom Quest "${quest.title}" for ${username}:`,
                  {
                    questId: quest.questId,
                ...
                if (quest.tasks) {
                  console.log(
                    `📋 [BATCH-CUSTOM-QUEST-TASKS] Tasks for "${quest.title}" (${username}):`
                  );
            ...
            const batchCustomQuests =
              questConfigWithRealTasks.questSequence.filter(
                (q) => q.questType === "custom"
              );
            const batchFixedQuests =
              questConfigWithRealTasks.questSequence.filter(
                (q) => q.questType === "fixed"
              );

            console.log(
              `🎯 [BATCH-BOT-SUMMARY] Bot will receive for ${username}:`,
              {
          ...
          if (
            response.ok &&
            data.results &&
            data.results.successful.length > 0
          ) {
            const successResult = data.results.successful[0];
            console.log(
              `✅ [CSV-BATCH] SUCCESS: ${username} -> ${successResult.repoName}`
            );
            ...
            const errorResult = data.results?.unsuccessful?.[0] || {
              error: data.message || "Unknown error",
            };
            console.error(
              `❌ [CSV-BATCH] FAILED: ${username} - ${errorResult.error}`
            );
            results.unsuccessful.push({
              ...
              details:
                errorResult.details || data.details || "No additional details",
              timestamp: new Date().toISOString(),
            });
          }
        } catch (error) {
          console.error(
            `💥 [CSV-BATCH] ERROR: ${username} - Network/processing error:`,
            error.message
          );
          results.unsuccessful.push({
            username,
      ...
      console.log(
        `📊 [CSV-BATCH] Final results: ${results.successful.length} successful, ${results.unsuccessful.length} failed`
      );
    } catch (error) {
      console.error(
        "💥 [CSV-BATCH] Critical error during batch processing:",
        error
      );
    ...
    <Box>
      ...
      >
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Create custom repositories for multiple users with the current quest
          configuration. Each user gets their own repository with the complete
          quest setup.
        </Typography>
        ...
        {(!customQuestConfig || !classInfo?.groupName) && (
          <Alert severity="warning" sx={{ mb: 2 }}>
            {!customQuestConfig &&
              "No quest configuration available. Please configure quests first before creating repositories."}
            {!classInfo?.groupName &&
                ...
                <List dense>
                  {batchResults.successful.map((result, index) => {
                    const userStatus = repositoryStatus
                      ? repositoryStatus.accepted?.includes(result.username)
                        ? "accepted"
                        : repositoryStatus.pending?.includes(result.username)
                        ? "pending"
                        : "not-found"
                      : null;
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body1" paragraph>
            This will create custom repositories for all users in the CSV file
            with the current quest configuration.
          </Typography>
          ...
          <Typography variant="body2" color="text.secondary">
            • Each user will get an individual repository
            <br />• Repository names will follow the pattern:{" "}
            <strong>
              {classInfo?.groupName?.toLowerCase().replace(/[^a-z0-9]+/g, "-")}
              -username
            </strong>
            <br />
            ...
            <br />• Failed creations will be reported with details
          </Typography>
