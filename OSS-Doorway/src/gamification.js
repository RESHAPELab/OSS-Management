...
import { handleMCQ, handleCustom, handleIssueCount, handlePRCount, handleTopContributor, handleIssueTitle, handleOpenIssues, handleTrueFalse, handleQuest, handleAssigned, handleIssueNo } from "./handlers/genericHandlers.js";
...
// Database function to fetch quests
async function getQuestsFromDatabase() {
  try {
    const quests = await Quest.find({}).populate('tasks');
    const questsData = {};
    
    for (const quest of quests) {
      questsData[quest.questKey] = {
        metadata: {
          title: quest.questTitle,
          prerequisite: quest.prerequisite && quest.prerequisite.length > 0 ? quest.prerequisite[0] : null
        }
      };
      
      // Add tasks for this quest
      for (const taskId of quest.tasks) {
        const task = await Task.findById(taskId);
        if (task) {
          questsData[quest.questKey][task.taskKey] = {
            desc: task.desc,
            points: task.points,
            xp: task.xp
          };
    ...
    // Add map_repo_link (you might want to store this in a config collection)
    questsData.map_repo_link = "https://raw.githubusercontent.com/caiton1/OSS-Doorway/main/map";
    
    return questsData;
  } catch (error) {
    console.error("Error fetching quests from database:", error);
    // Fallback to JSON file if database fails
    return JSON.parse(fs.readFileSync(questFilePath, "utf8"));
  }
}
...
async function connectToDatabase() {
  try {
    if (mongoose.connection.readyState === 0) {
      ...
      console.log("✅ Connected to MongoDB for quests");
    }
  } catch (error) {
...
// Function to initialize quests data
async function initializeQuests() {
  await connectToDatabase();
  quests = await getQuestsFromDatabase();
  console.log("Quests loaded from database:", quests);
}

// Initialize quests when module loads
initializeQuests().catch(console.error);
...
async function acceptQuest(context, user_data, quest) {
  ...
  try {
    // Ensure database connection and quests are loaded
    await connectToDatabase();
...
async function createRepos(context, org, users, db) {
  ...
  console.log(`🔍 Debug: OSS_REPO = ${process.env.OSS_REPO}`);
  console.log(`🔍 Debug: ossRepoData = [${ossRepoData[0]}, ${ossRepoData[1]}]`);

  // Check if there's a default README template available
  let defaultReadmeContent = null;
  try {
    defaultReadmeContent = fs.readFileSync(defaultReadmePath, "utf-8");
  } catch (error) {
    console.log("No default README template found, repositories will be created without README");
  }
  ...
  for (const username of users) {
    try {
      ...
      if (userResponse.status === 200 && !exists) {
        console.log(`🔍 Debug: User ${username} exists on GitHub and not in database`);
        
        // repo named after user
        console.log(`🔍 Debug: Creating repository: ${org}/${username}`);
        ...
        console.log(`✅ Repository created: ${org}/${username}`);

        // Wait a moment for repository to be fully created
        await new Promise(resolve => setTimeout(resolve, 2000));

        // invite user
        console.log(`🔍 Debug: Inviting user ${username} to their repo`);
        ...
        console.log(`🔍 Debug: Inviting user ${username} to OSS repo`);
        await context.octokit.repos.addCollaborator({
          owner: ossRepoData[0],
          repo: ossRepoData[1],
          username,
        ...
        // Add README file if default template is available
        if (defaultReadmeContent) {
          try {
            console.log(`📝 README COMMIT: Calling GitHub API to commit README.md to ${org}/${username}`);
            console.log(`📄 README COMMIT: File size: ${defaultReadmeContent.length} characters`);
            ...
            await context.octokit.repos.createOrUpdateFileContents({
              ...
              content: Buffer.from(defaultReadmeContent).toString("base64"),
              committer: {
                name: "QuestBuddy",
                email: "naugitbot@gmail.com",
              },
            ...
            console.log(`✅ README COMMIT: Successfully committed README.md to ${org}/${username}`);
          } catch (readmeError) {
            console.error(`❌ README COMMIT: Failed to commit README.md to ${org}/${username}:`, readmeError.message);
            // Continue with repo creation even if README fails
          }
        }

        // create issue in OSS Repo with non-code contribution tag
        console.log(`🔍 Debug: Creating issue in OSS repo: ${ossRepoData[0]}/${ossRepoData[1]}`);
        try {
          ...
          console.log(`✅ Successfully created issue in OSS repo for ${username}`);
        } catch (ossIssueError) {
          console.error(`❌ Failed to create issue in OSS repo for ${username}:`, ossIssueError.message);
          console.error(`❌ OSS repo details: ${ossRepoData[0]}/${ossRepoData[1]}`);
        }

        success.push(username);
      }
      
      // create issue inside new repo (only if repo was successfully created)
      if (success.includes(username)) {
        console.log(`🔍 Debug: Creating issue in user repo: ${org}/${username}`);
        try {
          ...
          console.log(`✅ Successfully created and closed setup issue for ${username}`);
        } catch (userIssueError) {
          console.error(`❌ Failed to create issue in user repo for ${username}:`, userIssueError.message);
        }
      }
...
async function getOriginalClassName(groupId) {
  try {
    ...
    // Connect to OSS-Management database
    const mongoose = require('mongoose');
    const ossManagementURI = process.env.OSS_MANAGEMENT_DB_URI || process.env.URI;
    const ossManagementDBName = process.env.OSS_MANAGEMENT_DB_NAME || process.env.DB_NAME;
    
    console.log(`[getOriginalClassName] Connecting to OSS-Management DB: ${ossManagementDBName}`);
    
    // Create a separate connection for OSS-Management database
    const ossManagementConnection = mongoose.createConnection(`${ossManagementURI}`);
    ...
    const Group = ossManagementConnection.model('Group', groupSchema);
    
    // Find the group by ID
    const group = await Group.findById(groupId);
    
    if (group && group.groupName) {
      console.log(`[getOriginalClassName] Found original class name: ${group.groupName}`);
      return group.groupName;
    } else {
      console.log(`[getOriginalClassName] Group not found or no groupName for groupId: ${groupId}`);
      return null;
    }
  } catch (error) {
