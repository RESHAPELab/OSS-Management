...
const questCompletion = async(req, res) => {
    try {
        ...
        const client = new MongoClient(process.env.OSS_DOORWAY_DB_URI);
...
const purpleDeployQuest = async (req, res) => {
    ...
    try {
        ...
        let studentsReadyForNewQuest = [];
        let reposWithNewIssues = []; // Track repos that just got issues created (for README update)
        ...
        try {
            ...
            // Ensure we have both legacy format (config) and modern format (questSequence)
            // This is critical for bot compatibility
            let legacyConfig = null;
            let finalQuestSequence = null;
            
            if (updatedConfig.questSequence && Array.isArray(updatedConfig.questSequence)) {
                // We have questSequence - convert to legacy format for bot compatibility
                finalQuestSequence = updatedConfig.questSequence;
                legacyConfig = {
                    map_repo_link: updatedConfig.map_repo_link || updatedConfig.config?.map_repo_link || "https://raw.githubusercontent.com/caiton1/OSS-Doorway/main/map"
                };
                
                // Convert questSequence to legacy format (Q1, Q2, etc.)
                for (const quest of updatedConfig.questSequence) {
                ...
                console.log(`✅ [PURPLE-DEPLOYMENT] Converted questSequence to legacy format: ${Object.keys(legacyConfig).filter(k => k.startsWith('Q')).join(', ')}`);
            } else if (updatedConfig.config) {
                // We have legacy format - extract questSequence if possible
                legacyConfig = updatedConfig.config;
                finalQuestSequence = updatedConfig.questSequence || null;
                
                // If no questSequence, try to build it from legacy format
                if (!finalQuestSequence) {
                    const questKeys = Object.keys(legacyConfig).filter(k => k.startsWith('Q')).sort();
                    finalQuestSequence = questKeys.map((questId, index) => {
                        const quest = legacyConfig[questId];
                        const tasks = {};
                    ...
                    console.log(`✅ [PURPLE-DEPLOYMENT] Built questSequence from legacy format: ${finalQuestSequence.length} quests`);
                }
            }
            
            // Remove MongoDB-specific fields from updatedConfig before spreading
            const { _id, createdAt, updatedAt, __v, ...cleanUpdatedConfig } = updatedConfig;
            ...
            const newConfig = {
                ...
                config: legacyConfig || updatedConfig.config,
                questSequence: finalQuestSequence || updatedConfig.questSequence,
                configData: {
                    map_repo_link: legacyConfig?.map_repo_link || updatedConfig.map_repo_link || "https://raw.githubusercontent.com/caiton1/OSS-Doorway/main/map",
                    questSequence: finalQuestSequence || updatedConfig.questSequence
                }
            };
            ...
            // Save the new quest configuration
            let insertResult;
            try {
                insertResult = await questConfigsCollection.insertOne(newConfig);
            console.log(`✅ [PURPLE-DEPLOYMENT] New quest configuration saved`);
                console.log(`   - Inserted ID: ${insertResult.insertedId}`);
                console.log(`   - Acknowledged: ${insertResult.acknowledged}`);
            } catch (insertError) {
                console.error(`❌ [PURPLE-DEPLOYMENT] Failed to insert config:`, insertError);
                console.error(`   - Config classId: ${newConfig.classId}`);
                console.error(`   - Config keys: ${Object.keys(newConfig).join(', ')}`);
                throw new Error(`Failed to save purple config: ${insertError.message}`);
            }
            ...
            try {
                ...
                if (verifyConfig) {
                    console.log(`✅ [PURPLE-DEPLOYMENT] Verification: Config exists in database`);
                    console.log(`   - Verified _id: ${verifyConfig._id}`);
                    console.log(`   - Quest count (questSequence): ${verifyConfig.questSequence?.length || 0}`);
                    console.log(`   - Quest count (legacy): ${verifyConfig.config ? Object.keys(verifyConfig.config).filter(k => k.startsWith('Q')).length : 0}`);
                } else {
                    console.error(`❌ [PURPLE-DEPLOYMENT] VERIFICATION FAILED: Config not found after save!`);
                    console.error(`   - Searched for classId: ${newConfig.classId}`);
                    throw new Error('Config was not saved - verification failed');
                }
            } catch (verifyError) {
                console.error(`❌ [PURPLE-DEPLOYMENT] Verification error:`, verifyError);
                // Don't fail the deployment if verification fails, but log it
            }
            ...
            // Use the quest's configured prerequisite (respects quest chain dependencies)
            // Only fall back to highest completed quest if no prerequisite is configured
            const actualPrerequisiteQuestId = newQuest.metadata.prerequisite || null;
            console.log(`🎯 [PURPLE-DEPLOYMENT] Quest prerequisite: ${actualPrerequisiteQuestId} (from config: ${newQuest.metadata.prerequisite})`);
            ...
            try {
                ...
                // AUTO-UNLOCK: Now unlock the new quest ONLY for users who completed the prerequisite
                {
                    // Use studentsReadyForNewQuest instead of all migrated users
                    const targetUsers = studentsReadyForNewQuest || [];
                    console.log(`🚀 [PURPLE-DEPLOYMENT] Step 12: Auto-accepting ${newQuestId} for ${targetUsers.length} users who completed prerequisite`);
                    console.log(`📊 [PURPLE-DEPLOYMENT] Total migrated users: ${usersWithNewQuestAccess.length}, Users ready for ${newQuestId}: ${targetUsers.length}`);
                    
                    let autoUnlockSuccess = 0;
                    
                    try {
                        // Import unlockQuestForUser from repoController (direct DB update + issue creation, no comments)
                        // Note: unlockQuestForUser creates its own DB connection and GitHub auth, so we don't need accessToken here
                        const { unlockQuestForUser } = require('./repoController');
                        console.log(`✅ [PURPLE-DEPLOYMENT] Using direct unlockQuestForUser (no comments needed)`);
                        
                        // Get legacy config format for unlockQuestForUser
                        // Prefer existing config if available, otherwise convert from questSequence
                        let legacyQuestConfig = null;
                        if (newConfig.config && typeof newConfig.config === 'object') {
                            // Use existing legacy config (already converted during purple deploy)
                            legacyQuestConfig = newConfig.config;
                            console.log(`✅ [PURPLE-DEPLOYMENT] Using existing legacy config format`);
                        } else if (newConfig.questSequence && Array.isArray(newConfig.questSequence)) {
                            // Convert questSequence format to legacy config format for unlockQuestForUser
                            legacyQuestConfig = {
                                map_repo_link: newConfig.map_repo_link || "https://raw.githubusercontent.com/caiton1/OSS-Doorway/main/map"
                            };
                            newConfig.questSequence.forEach((quest) => {
                                const questId = quest.questId || quest.metadata?.questId;
                                if (questId) {
                                    legacyQuestConfig[questId] = {
                                        metadata: quest.metadata || {},
                                        ...quest.tasks
                                    };
                                }
                            });
                            console.log(`✅ [PURPLE-DEPLOYMENT] Converted questSequence to legacy config format`);
                        } else {
                            throw new Error('No valid quest configuration found in newConfig (neither config nor questSequence)');
                        }
                        
                        let successCount = 0;
                        let unlockErrors = [];
                        reposWithNewIssues = []; // Reset for this deployment
                        
                        for (const student of targetUsers) {
                            try {
                                // student is an object with username and repo properties
                                const username = student.username || student;
                                const repo = student.repo || student; // For OSS-Doorway repos, username is the repo name
                                console.log(`🔓 [PURPLE-DEPLOYMENT] Directly unlocking ${newQuestId} for ${username} (no comments, direct DB update)...`);
                                
                                // Call unlockQuestForUser which directly updates DB and creates issues (no /accept comments needed)
                                const result = await unlockQuestForUser(
                                    newQuestId,
                                    username,
                                    repo,
                                    legacyQuestConfig,
                                ...
                                if (result && result.success) {
                                    successCount++;
                                    console.log(`✅ [PURPLE-DEPLOYMENT] Successfully unlocked ${newQuestId} for ${username} (${result.tasksCreated} issues created)`);
                                    // Track repos that just got issues created for README update
                                    if (result.tasksCreated > 0) {
                                        reposWithNewIssues.push(repo);
                                    }
                                } else {
                                    console.log(`⚠️ [PURPLE-DEPLOYMENT] Quest ${newQuestId} unlock returned: ${JSON.stringify(result)}`);
                                    unlockErrors.push(`${username}: Unlock returned non-success result`);
                                }
                            } catch (studentError) {
                                const username = student.username || student;
                                const errorMsg = `${username}: ${studentError.message}`;
                                console.error(`❌ [PURPLE-DEPLOYMENT] Error processing ${username}:`, studentError.message);
                                unlockErrors.push(errorMsg);
                            }
                        }
                        
                        // Add delay to let GitHub API index newly created issues before README update
                        if (reposWithNewIssues.length > 0) {
                            console.log(`⏳ [PURPLE-DEPLOYMENT] Waiting 3 seconds for GitHub API to index ${reposWithNewIssues.length} newly created issue(s)...`);
            ...
            // Post-migration: trigger dynamic README update for all student repos
            // This ensures quest links, SVG scorecards, and progress are displayed correctly
            try {
                console.log(`📝 [PURPLE-DEPLOYMENT] Starting dynamic README update for ${usersToMigrate.length} student repos...`);
                
                if (!accessToken) {
                    console.log(`⚠️ [PURPLE-DEPLOYMENT] No GitHub access token for README updates, skipping`);
                } else {
                    let readmeUpdateCount = 0;
                    const readmeUpdateErrors = [];
                    
                    // Trigger bot's updateReadme for each student by posting /updatereadme command
                    for (const user of usersToMigrate) {
                        try {
                            const repo = user._id;
                            
                            // For repos that just got issues created, try to get issue number from database first
                            let targetIssueNumber = null;
                            if (reposWithNewIssues.includes(repo)) {
                                try {
                                    // Query database to get the issue number that was just created
                                    const tempClient = new MongoClient(ossDoorwayUri);
                                    await tempClient.connect();
                                    const tempDb = tempClient.db(ossDoorwayDbName);
                                    const tempCollection = tempDb.collection('user_data');
                                    const userDoc = await tempCollection.findOne({ _id: repo });
                                    await tempClient.close();
                                    
                                    if (userDoc && userDoc.user_data && userDoc.user_data.accepted && userDoc.user_data.accepted[newQuestId]) {
                                        // Find the first task with an issue number
                                        for (const taskKey in userDoc.user_data.accepted[newQuestId]) {
                                            if (taskKey !== 'metadata' && userDoc.user_data.accepted[newQuestId][taskKey].issueNum > 0) {
                                                targetIssueNumber = userDoc.user_data.accepted[newQuestId][taskKey].issueNum;
                                                console.log(`📝 [PURPLE-DEPLOYMENT] Found newly created issue #${targetIssueNumber} for ${repo} from database`);
                                                break;
                                            }
                                        }
                                    }
                                } catch (dbError) {
                                    console.log(`⚠️ [PURPLE-DEPLOYMENT] Could not get issue number from DB for ${repo}, will query GitHub API: ${dbError.message}`);
                                }
                            }
                            
                            // If we have the issue number from DB, use it directly
                            if (targetIssueNumber) {
                                try {
                                    console.log(`📝 [PURPLE-DEPLOYMENT] Posting /updatereadme to ${repo}#${targetIssueNumber} (from database)`);
                                    await axios.post(`https://api.github.com/repos/OSS-Doorway-Dev/${repo}/issues/${targetIssueNumber}/comments`, {
                                        body: `/updatereadme`
                                    }, {
                                        headers: {
                                            'Authorization': `token ${accessToken}`,
                                    ...
                                    readmeUpdateCount++;
                                    console.log(`✅ [PURPLE-DEPLOYMENT] README update triggered for ${repo}`);
                                    continue;
                                } catch (issueError) {
                                    console.log(`⚠️ [PURPLE-DEPLOYMENT] Failed to post to issue #${targetIssueNumber}, falling back to API query: ${issueError.message}`);
                                }
                            }
                            
                            // Fallback: Find their most recent open issue via GitHub API
                            ...
                            if (issuesResponse.data && issuesResponse.data.length > 0) {
                                const issue = issuesResponse.data[0];
                                console.log(`📝 [PURPLE-DEPLOYMENT] Posting /updatereadme to ${repo}#${issue.number} (from GitHub API)`);
                                
                                await axios.post(`https://api.github.com/repos/OSS-Doorway-Dev/${repo}/issues/${issue.number}/comments`, {
                                    body: `/updatereadme`
                                }, {
                                    headers: {
                                        'Authorization': `token ${accessToken}`,
                                ...
                                if (closedIssuesResponse.data && closedIssuesResponse.data.length > 0) {
                                    const issue = closedIssuesResponse.data[0];
                                    console.log(`📝 [PURPLE-DEPLOYMENT] Posting /updatereadme to ${repo}#${issue.number} (closed issue fallback)`);
                                    await axios.post(`https://api.github.com/repos/OSS-Doorway-Dev/${repo}/issues/${issue.number}/comments`, {
                                        body: `/updatereadme`
                                    }, {
                                        headers: {
                                    ...
                                    readmeUpdateCount++;
                                    console.log(`✅ [PURPLE-DEPLOYMENT] README update triggered for ${repo} (via closed issue)`);
                                } else {
                                    console.log(`⚠️ [PURPLE-DEPLOYMENT] No issues found (open or closed) for ${repo}, skipping README update`);
                                }
                            }
                        } catch (readmeError) {
                            const errorMsg = `${user._id}: ${readmeError.message}`;
                            console.error(`❌ [PURPLE-DEPLOYMENT] README update failed for ${user._id}:`, readmeError.message);
                            readmeUpdateErrors.push(errorMsg);
                        }
                    }
                    
                    console.log(`✅ [PURPLE-DEPLOYMENT] README update triggered for ${readmeUpdateCount}/${usersToMigrate.length} repos`);
                    if (readmeUpdateErrors.length > 0) {
                        console.log(`⚠️ [PURPLE-DEPLOYMENT] README update errors:`, readmeUpdateErrors);
                    }
                }
            } catch (batchErr) {
