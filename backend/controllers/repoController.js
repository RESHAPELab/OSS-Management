...
Repository for students in ${className}.`;
    ...
    if (customSequenceData && customSequenceData.questSequence && customSequenceData.questSequence.length > 0) {
      ...
      // List all available quests and tasks; link only up to buffer size for the FIRST quest
      if (Array.isArray(customSequenceData.questSequence)) {
        const taskBufferSize = parseInt(process.env.TASK_BUFFER_SIZE) || 1;
        
        // Identify the first quest (no prerequisite or isQ0)
        ...
        for (const quest of customSequenceData.questSequence) {
          ...
          const hasPrerequisite = quest.metadata?.prerequisite;
          const isQ0 = quest.metadata?.isQ0;
          
          if (!hasPrerequisite || isQ0) {
            firstQuestId = questId;
            console.log(`📝 [README-GENERATION] Identified first quest for initial links: ${firstQuestId}`);
            break;
          }
        }
        
        // Fallback: if no quest without prerequisite found, use the first quest in sequence
        if (!firstQuestId && customSequenceData.questSequence.length > 0) {
          firstQuestId = customSequenceData.questSequence[0].questId || customSequenceData.questSequence[0].metadata?.questId;
          console.log(`📝 [README-GENERATION] Using fallback first quest: ${firstQuestId}`);
        }
        ...
        for (const quest of customSequenceData.questSequence) {
          ...
          const isFirstQuest = questId === firstQuestId;
          ...
          if (quest.tasks && typeof quest.tasks === 'object') {
            ...
            taskEntries.forEach(([taskKey, taskVal], index) => {
              ...
              // Only create links for tasks in the first quest, up to buffer size
              if (isFirstQuest && index < taskBufferSize) {
              ...
              const tLower = String(taskKey).toLowerCase();
                ...
                // All other tasks: show without links
                progressSection += `  - ${taskKey} - ${taskDesc}\n`;
              }
            });
          }
            ...
            if (firstQuest) {
              ...
              if (!isDefaultSequence) {
                ...
                if (enhancedQuestsEnabled) {
                  if (isPurpleConfig) {
                    // Purple config: Unlock ONLY first quest (sequential unlocking via completeQuest)
                    console.log(`🌟 [PURPLE-DEPLOY] Enhanced Quest System: Unlocking first quest for ${repoName}`);
                    
                    // Find the first quest (no prerequisite or isQ0) - same logic as regular repo
                    let firstQuestForPurple = null;
                    
                    // Try to find quest with no prerequisite or isQ0
                    for (const questKey of Object.keys(questConfig)) {
                      if (questKey === 'map_repo_link') continue;
                      const questData = questConfig[questKey];
                      if (questData && typeof questData === 'object') {
                        if (!questData.metadata || !questData.metadata.prerequisite || questData.metadata.isQ0) {
                          firstQuestForPurple = questKey;
                          console.log(`🌟 [PURPLE-DEPLOY] Found first quest: ${questKey}`);
                          break;
                        }
                      }
                    }
                    
                    // Fallback: use Q0, Q1, or first available quest
                    if (!firstQuestForPurple) {
                      const allQuestIds = Object.keys(questConfig).filter(key => key !== 'map_repo_link').sort();
                      if (allQuestIds.includes('Q0')) {
                        firstQuestForPurple = 'Q0';
                      } else if (allQuestIds.includes('Q1')) {
                        firstQuestForPurple = 'Q1';
                      } else if (allQuestIds.length > 0) {
                        firstQuestForPurple = allQuestIds[0];
                      }
                      console.log(`🌟 [PURPLE-DEPLOY] Using fallback first quest: ${firstQuestForPurple}`);
                    }
                    
                    if (firstQuestForPurple) {
                          try {
                        const result = await unlockQuestForUser(firstQuestForPurple, user, repoName, questConfig, uniqueGroupId, className);
                            if (result && result.success) {
                          console.log(`🌟 [PURPLE-DEPLOY] ✅ Unlocked ${firstQuestForPurple} for ${user} (${result.tasksCreated}/${result.tasksCreated + result.tasksFailed} tasks)`);
                          console.log(`🌟 [PURPLE-DEPLOY] 🔄 Subsequent quests will unlock sequentially via completeQuest`);
                            } else {
                          console.log(`🌟 [PURPLE-DEPLOY] ❌ Failed to unlock ${firstQuestForPurple} for ${user}`);
                            }
                          } catch (error) {
                        console.error(`🌟 [PURPLE-DEPLOY] ❌ Error unlocking ${firstQuestForPurple} for ${user}:`, error.message);
                      }
                    } else {
                      console.warn(`🌟 [PURPLE-DEPLOY] ⚠️ No first quest found for ${repoName}`);
                    }
                  } else {
