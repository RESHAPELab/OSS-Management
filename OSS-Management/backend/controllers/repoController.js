...
const listOrganizationRepos = async (req, res) => {
    try {
        ...
        console.log('[BACKEND] Received organizationGh:', organizationGh);
        if (!organizationGh) {
            return res.status(400).json({ message: "Invalid request. Required: organizationGh" });
        }
        // Use POST to call the bot, passing org in the body
        const response = await sendMessageToBot(
            "github/listRepos",
            { org: organizationGh }
        );
        console.log('[BACKEND] Sent org to bot:', organizationGh);
        res.status(200).json({
