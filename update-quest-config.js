...
async function updateQuestConfig() {
  try {
    ...
    const GroupModel = mongoose.model('Group', new mongoose.Schema({}, { strict: false }));

    // Update all groups with the new quest configuration
    ...
    console.log(`✅ Updated ${result.modifiedCount} group(s) with new quest configuration`);
    console.log('📊 Configuration includes:');
    console.log('   - Quest Q1 with 6 tasks');
    ...
    console.error('❌ Error updating quest configuration:', error);
    process.exit(1);
  }
}
