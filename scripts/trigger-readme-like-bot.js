...
// Import the bot's gamification functions
...
async function triggerReadmeUpdateLikeBotDoes() {
  console.log('🚀 Triggering README updates exactly like the bot does after task completion...');
  ...
  for (let i = 0; i < Q6_USERS.length; i++) {
    ...
    try {
      // Create a webhook payload that simulates a task completion, which triggers updateReadme
      const webhookPayload = {
        action: 'created',
        issue: {
          number: 1,
          title: 'Trigger README update',
          body: 'README update needed',
          user: {
            login: repoName.split('-')[0] // Extract username from repo name
          }
        },
        comment: {
          body: '/bypass Q6.T1', // This will complete a task and trigger README update
          user: {
      ...
      const response = await axios.post(BOT_WEBHOOK_URL, webhookPayload, {
        headers: {
          ...
          'X-GitHub-Delivery': `update-readme-${Date.now()}-${i}`,
