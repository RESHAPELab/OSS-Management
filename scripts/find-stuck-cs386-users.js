...
async function findStuckCS386Users() {
  ...
  try {
    ...
    // Get quest config for CS386 class
    const cs386Config = await db.collection('questconfigs').findOne({
      configId: '68a770b8140b9c0174c13ce7'
    });
    
    if (!cs386Config) {
      console.log('❌ CS386 quest config not found');
      return;
    }
    
    console.log(`📋 CS386 Quest Config Found:`);
    console.log(`   Config ID: ${cs386Config.configId}`);
    console.log(`   Available Quests: ${Object.keys(cs386Config.config).filter(k => k.startsWith('Q')).join(', ')}`);
    
    // Get all users with CS386 customGroupId
    ...
    for (const user of cs386Users) {
      ...
      if (userData.current) {
        ...
        if (userData.accepted && 
          ...
          // Find next task
          const questConfig = cs386Config.config[currentQuest];
          if (questConfig) {
...
// Run the script
findStuckCS386Users().catch(console.error);
